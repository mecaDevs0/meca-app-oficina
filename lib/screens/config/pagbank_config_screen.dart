...
class _PagBankConfigScreenState extends State<PagBankConfigScreen> {
        ...
        children: [
          ...
          if (hasAccount && account != null) ...[
            ...
            Container(
              ...
              child: Column(
                children: [
                  _buildInfoRow('Nome', account['name'] ?? 'N/A'),
                  _buildInfoRow('Email', account['email'] ?? 'N/A'),
                  _buildInfoRow('Documento', account['document'] ?? 'N/A'),
                  _buildInfoRow('Status', account['status'] ?? 'N/A'),
                ],
              ),
            ),
          ],
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          if (account != null) ...[
            _buildInfoRow('ID da Conta', account['id'] ?? 'N/A'),
            _buildInfoRow('Tipo', account['type'] ?? 'N/A'),
            _buildInfoRow('Criado em', _formatDate(account['created_at'])),
            _buildInfoRow('Atualizado em', _formatDate(account['updated_at'])),
          ],
        ],
      ),
    );
  ...
  Color _getStatusColor(String? status) {
    switch (status) {
      ...
      default:
        return const secondaryTextColor;
    }
  }
