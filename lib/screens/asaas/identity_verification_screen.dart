...
class _IdentityVerificationScreenState
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (res['success'] == true && res['data'] != null) {
        ...
        _safeSetState(() {
          _errorMessage =
              res['error']?.toString() ?? 'Erro ao carregar dados de verificação.';
        });
      }
    } catch (e) {
      _safeSetState(() => _errorMessage = 'Erro de conexão: ${e.toString()}');
    } finally {
          ...
          if (isNotSent || statusUpper == 'REJECTED')
            Padding(
              ...
              child: requiresExternalLink
                  ? (onboardingUrl != null && onboardingUrl.isNotEmpty
                      ? _buildExternalLinkButton(onboardingUrl, isDark)
                      : _buildAwaitingLinkMessage(isDark))
  ...
  Widget _buildAwaitingLinkMessage(bool isDark) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(
        color: AppColors.yellowWarningColor.withOpacity(0.1),
        borderRadius: BorderRadius.circular(10),
        border: Border.all(color: AppColors.yellowWarningColor.withOpacity(0.3)),
      ),
      child: Row(
        children: [
          Icon(Icons.info_outline, color: AppColors.yellowWarningColor, size: 18),
          const SizedBox(width: 10),
          Expanded(
            child: Text(
              'Este documento requer verificação pelo link do Asaas. Aguarde a liberação ou entre em contato com o suporte.',
              style: TextStyle(
                color: AppColors.yellowWarningColor,
                fontSize: 12,
                height: 1.4,
  ...
  Widget _buildExternalLinkButton(String url, bool isDark) {
    return SizedBox(
      width: double.infinity,
      height: 48,
